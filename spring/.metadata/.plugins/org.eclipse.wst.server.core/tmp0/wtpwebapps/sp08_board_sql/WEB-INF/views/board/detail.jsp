...
<div class="container">
	<%-- 	${resultMap.bdto}
			${resultMap.bflist}--%>
	<h2>상세조회</h2>
	번호 : <input type="text" id="bnum" value="${resultMap.bdto.bnum}" readonly="readonly"> 
	조회수 :  ${resultMap.bdto.readcount} <br>
	...
	<hr>	
	<button id="btnLike" class="btn btn-primary">좋아요</button><span id="likecnt">${resultMap.bdto.likecnt}</span>
	<button id="btnDisLike" class="btn btn-danger">싫어요</button><span id="dislikecnt">${resultMap.bdto.dislikecnt}</span>
	<hr>	
	등록일자 :  ${resultMap.bdto.regdate} <br>
	수정일자 :  ${resultMap.bdto.modifydate} <br>
	<hr>	
	<button id="btnModify" class="btn btn-success">수정</button>
	<button id="bthReply" class="btn btn-info">댓글</button>
